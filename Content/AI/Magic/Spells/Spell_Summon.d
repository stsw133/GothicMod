...
{
	if ((Npc_GetActiveSpellIsScroll(self) && (self.attribute[ATR_MANA] >= SPL_Cost_Summon/5))
...
func void Spell_Cast_Summon()
{
	if (Npc_GetActiveSpellIsScroll(self) && (self.attribute[ATR_MANA] >= SPL_Cost_Summon/5))
	{
		self.attribute[ATR_MANA] -= SPL_Cost_Summon/5;
	}
	else if (self.attribute[ATR_MANA] >= SPL_Cost_Summon)
	{
		self.attribute[ATR_MANA] -= SPL_Cost_Summon;
	};
