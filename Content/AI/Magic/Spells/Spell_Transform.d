...
{
	if ((Npc_GetActiveSpellIsScroll(self) && (self.attribute[ATR_MANA] >= SPL_Cost_Transform/5))
...
func void Spell_Cast_Transform()
{
	if (Npc_GetActiveSpellIsScroll(self) && (self.attribute[ATR_MANA] >= SPL_Cost_Transform/5))
	{
		self.attribute[ATR_MANA] -= SPL_Cost_Transform/5;
	}
	else if (self.attribute[ATR_MANA] >= SPL_Cost_Transform)
	{
		self.attribute[ATR_MANA] -= SPL_Cost_Transform;
	};
