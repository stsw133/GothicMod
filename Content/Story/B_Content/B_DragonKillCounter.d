...
func int B_DragonKillCounter (var C_NPC current_dragon)
{
	...
	if (current_dragon.guild == GIL_DRAGON)
	{
		...
		{	B_SetMonsterAttributes (SwampDragon, 35+(MIS_KilledDragons*5));	};
		if (!Npc_IsDead(RockDragon))
		{	B_SetMonsterAttributes (RockDragon, 35+(MIS_KilledDragons*5));	};
		if (!Npc_IsDead(FireDragon))
		{	B_SetMonsterAttributes (FireDragon, 35+(MIS_KilledDragons*5));	};
		if (!Npc_IsDead(IceDragon))
		{	B_SetMonsterAttributes (IceDragon, 35+(MIS_KilledDragons*5));	};
		if (!Npc_IsDead(GoldDragon))
		{	B_SetMonsterAttributes (GoldDragon, 35+(MIS_KilledDragons*5));	};
		if (!Npc_IsDead(FireDragonIsland))
		{	B_SetMonsterAttributes (FireDragonIsland, 35+(MIS_KilledDragons*5));	};
		if (!Npc_IsDead(UndeadDragon))
		{	B_SetMonsterAttributes (UndeadDragon, 35+(MIS_KilledDragons*5));	};
	};
