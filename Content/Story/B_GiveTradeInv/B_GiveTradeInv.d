...
func void B_ClearDeadTrader (var C_NPC slf)
{
	...
	slf.aivar[AIV_ChapterInv] = 0-Kapitel;
};
...
func void B_GiveTradeInv (var C_NPC slf)
{
	...
	{
		// ADDON
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Cronos_ADW))	{	B_ClearJunkInv(slf); B_GiveTradeInv_Addon_Cronos_ADW(slf);		};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Cronos_NW))		{	B_ClearJunkInv(slf); B_GiveTradeInv_Addon_Cronos_NW(slf);		};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Erol))			{	B_ClearJunkInv(slf); B_GiveTradeInv_Addon_Erol(slf);			};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Fisk))			{	B_ClearJunkInv(slf); B_GiveTradeInv_Addon_Fisk(slf);			};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Garett))		{	B_ClearJunkInv(slf); B_GiveTradeInv_Addon_Garett(slf);			};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Huno))			{	B_ClearJunkInv(slf); B_GiveTradeInv_Addon_Huno(slf);			};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Juan))			{	B_ClearJunkInv(slf); B_GiveTradeInv_Addon_Juan(slf);			};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Lucia))			{	B_ClearJunkInv(slf); B_GiveTradeInv_Addon_Lucia(slf);			};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Martin))		{	B_ClearJunkInv(slf); B_GiveTradeInv_Addon_Martin(slf);			};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Miguel))		{	B_ClearJunkInv(slf); B_GiveTradeInv_Addon_Miguel(slf);			};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Samuel))		{	B_ClearJunkInv(slf); B_GiveTradeInv_Addon_Samuel(slf);			};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Scatty))		{	B_ClearJunkInv(slf); B_GiveTradeInv_Addon_Scatty(slf);			};
		
		// GOTHIC2		
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Baltram))		{	B_ClearJunkInv(slf); B_GiveTradeInv_Baltram(slf);				};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Bennet))		{	B_ClearJunkInv(slf); B_GiveTradeInv_Bennet(slf);				};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Bennet_DI))		{	B_ClearJunkInv(slf); B_GiveTradeInv_Bennet_DI(slf);				};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Bosper))		{	B_ClearJunkInv(slf); B_GiveTradeInv_Bosper(slf);				};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Brahim))		{	B_ClearJunkInv(slf); B_GiveTradeInv_Brahim(slf);				};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Brian))			{	B_ClearJunkInv(slf); B_GiveTradeInv_Brian(slf);					};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Canthar))		{	B_ClearJunkInv(slf); B_GiveTradeInv_Canthar(slf);				};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Constantino))	{	B_ClearJunkInv(slf); B_GiveTradeInv_Constantino(slf);			};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Coragon))		{	B_ClearJunkInv(slf); B_GiveTradeInv_Coragon(slf);				};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Diego_DI))		{	B_ClearJunkInv(slf); B_GiveTradeInv_Diego_DI(slf);				};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Elena))			{	B_ClearJunkInv(slf); B_GiveTradeInv_Elena(slf);					};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Engor))			{	B_ClearJunkInv(slf); B_GiveTradeInv_Engor(slf);					};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Fenia))			{	B_ClearJunkInv(slf); B_GiveTradeInv_Fenia(slf);					};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Gaertner))		{	B_ClearJunkInv(slf); B_GiveTradeInv_Gaertner(slf);				};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Gorax))			{	B_ClearJunkInv(slf); B_GiveTradeInv_Gorax(slf);					};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Hakon))			{	B_ClearJunkInv(slf); B_GiveTradeInv_Hakon(slf);					};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Halvor))		{	B_ClearJunkInv(slf); B_GiveTradeInv_Halvor(slf);				};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Harad))			{	B_ClearJunkInv(slf); B_GiveTradeInv_Harad(slf);					};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Hodges))		{	B_ClearJunkInv(slf); B_GiveTradeInv_Hodges(slf);				};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Ignaz))			{	B_ClearJunkInv(slf); B_GiveTradeInv_Ignaz(slf);					};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Isgaroth))		{	B_ClearJunkInv(slf); B_GiveTradeInv_Isgaroth(slf);				};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Jora))			{	B_ClearJunkInv(slf); B_GiveTradeInv_Jora(slf);					};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Kardif))		{	B_ClearJunkInv(slf); B_GiveTradeInv_Kardif(slf);				};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Karras))		{	B_ClearJunkInv(slf); B_GiveTradeInv_Karras(slf);				};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Khaled))		{	B_ClearJunkInv(slf); B_GiveTradeInv_Khaled(slf);				};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Kjorn))			{	B_ClearJunkInv(slf); B_GiveTradeInv_Kjorn(slf);					};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Lutero))		{	B_ClearJunkInv(slf); B_GiveTradeInv_Lutero(slf);				};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Matteo))		{	B_ClearJunkInv(slf); B_GiveTradeInv_Matteo(slf);				};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Milten_DI))		{	B_ClearJunkInv(slf); B_GiveTradeInv_Milten_DI(slf);				};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Orlan))			{	B_ClearJunkInv(slf); B_GiveTradeInv_Orlan(slf);					};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Rethon))		{	B_ClearJunkInv(slf); B_GiveTradeInv_Rethon(slf);				};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Rosi))			{	B_ClearJunkInv(slf); B_GiveTradeInv_Rosi(slf);					};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Sagitta))		{	B_ClearJunkInv(slf); B_GiveTradeInv_Sagitta(slf);				};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Salandril))		{	B_ClearJunkInv(slf); B_GiveTradeInv_Salandril(slf);				};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Sarah))			{	B_ClearJunkInv(slf); B_GiveTradeInv_Sarah(slf);					};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Sengrath))		{	B_ClearJunkInv(slf); B_GiveTradeInv_Sengrath(slf);				};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Tandor))		{	B_ClearJunkInv(slf); B_GiveTradeInv_Tandor(slf);				};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Vatras_DI))		{	B_ClearJunkInv(slf); B_GiveTradeInv_Vatras_DI(slf);				};
		if (Hlp_GetInstanceID(slf) == Hlp_GetInstanceID(Trd_Zuris))			{	B_ClearJunkInv(slf); B_GiveTradeInv_Zuris(slf);					};
		
		// NEW
		//if (Hlp_GetInstanceID(slf) ==
