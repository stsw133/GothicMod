...
FUNC VOID B_ENTER_NEWWORLD_Kapitel_2()
{
	...
	{
	 	Wld_InsertNpc (Skeleton_Gobbo, "FP_ROAM_MEDIUMFOREST_KAP2_24");
		...
		{	Wld_InsertItem (ItAm_Eternity_01, "FP_ROAM_XARDAS_SECRET_26");	};
...
FUNC VOID B_ENTER_NEWWORLD_Kapitel_3()
{
	if (EnterNW_Kapitel3 == false)
	{
		...
		if (Npc_IsDead(Malak) == false)
		{
			...
			if (hero.guild == GIL_KDF)
			{	CreateInvItem (Malak, ITWR_DementorObsessionBook_MIS);	};
		};
		...
		if (hero.guild == GIL_KDF)
		{
			...
			if (Npc_IsDead(Vino) == false)
			{
				...
				CreateInvItem (Vino, ITWR_DementorObsessionBook_MIS);
			...
			{	CreateInvItem (Bromor, ITWR_DementorObsessionBook_MIS);	};
		};
		...
		&& (Npc_IsDead(Canthar) == false)
		{
 		...
 		CreateInvItem (Lester, ItMw_1h_Bau_Mace);
 		CreateInvItem (Ehnim, ItMi_Moleratlubric_MIS);
...
FUNC VOID B_ENTER_NEWWORLD_Kapitel_4()
{
	if (EnterNW_Kapitel4 == false)
	{
		...
		if (hero.guild == GIL_KDF)
		{
			...
			CreateInvItem (Randolph, ITWR_DementorObsessionBook_MIS);
...
FUNC VOID B_ENTER_NEWWORLD_Kapitel_5()
{
	if (EnterNW_Kapitel5 == false)
	{
		...
		if (hero.guild == GIL_KDF)
		{
			...
			if (Npc_IsDead(Sekob) == false)
			{
				CreateInvItem (Sekob, ITWR_DementorObsessionBook_MIS);
...
///******************************************************************************************
///	B_ENTER_NEWWORLD (wird über INIT_NEWWORLD)
///******************************************************************************************
FUNC VOID B_ENTER_NEWWORLD()	
{
	B_InitNpcGlobals (); 
	if (Kapitel >= 7)	{	B_ENTER_NEWWORLD_Kapitel_1();	};
	if (Kapitel >= 8)	{	B_ENTER_NEWWORLD_Kapitel_2();	};
	if (Kapitel >= 9)	{	B_ENTER_NEWWORLD_Kapitel_3();	};
	if (Kapitel >= 10)	{	B_ENTER_NEWWORLD_Kapitel_4();	};
	if (Kapitel >= 11)	{	B_ENTER_NEWWORLD_Kapitel_5();	};
	if (Kapitel >= 12)	{	B_ENTER_NEWWORLD_Kapitel_6();	};
