...
FUNC VOID B_ENTER_OLDWORLD_Kapitel_4()
{
	if (EnterOW_Kapitel4 == false)
	{
		if ((Npc_IsDead(Engrom)) == false)
		{
			...
			CreateInvItem (Engrom, ItAt_TalbinsLurkerSkin);
			if (hero.guild == GIL_KDF)
			{
				CreateInvItem (Engrom, ITWR_DementorObsessionBook_MIS);
			}
			else
			{
				B_KillNpc(Engrom);
		...
		if ((Npc_IsDead(Sengrath)) == false)
		{
			...
			if (Npc_HasItems(Sengrath,ItRw_Mil_Crossbow))
			{
	 		...
	 		CreateInvItem (Sengrath, ItRw_SengrathsArmbrust_MIS);
		...
		Wld_InsertNpc (Waran, "OW_PATH_182");

		Wld_InsertNpc (Waran, "FP_ROAM_OW_SCAVENGER_01_07");
		Wld_InsertNpc (Waran, "FP_ROAM_OW_SCAVENGER_01_06");
		Wld_InsertNpc (Waran, "OW_PATH_182");
		...
		Wld_InsertNpc (Waran, "PLATEAU_ROUND02_CAVE");
...
//		Wld_InsertItem (ItSc_Pyrokinesis, "FP_ROAM_OW_ROCK_DRACONIAN_07_2");
		...
		//IntroduceChapter (ConcatStrings(TEXT_Chapter,IntToString(4)), TEXT_Chapter_04, "chapter_04.tga", "chapter_01.wav", 6000);
...
FUNC VOID B_ENTER_OLDWORLD_Kapitel_5()
{
	...
	{
		CreateInvItem (VLK_4143_HaupttorWache, ITKE_OC_MAINGATE_MIS);

		if (Npc_IsDead(Brutus) == false)
		{	CreateInvItem (VLK_4100_Brutus, ITWR_DementorObsessionBook_MIS);	};
...
///******************************************************************************************
///	B_ENTER_OLDWORLD (wird über INIT_OLDWORLD)
///******************************************************************************************
FUNC VOID B_ENTER_OLDWORLD()	
{
	B_InitNpcGlobals();
	if (Kapitel >= 7)	{	B_ENTER_OLDWORLD_Kapitel_1();	};
	if (Kapitel >= 8)	{	B_ENTER_OLDWORLD_Kapitel_2();	};
	if (Kapitel >= 9)	{	B_ENTER_OLDWORLD_Kapitel_3();	};
	if (Kapitel >= 10)	{	B_ENTER_OLDWORLD_Kapitel_4();	};
	if (Kapitel >= 11)	{	B_ENTER_OLDWORLD_Kapitel_5();	};
	if (Kapitel >= 12)	{	B_ENTER_OLDWORLD_Kapitel_6();	};
