...
func void DIA_Pepe_KillWolves_Info()
{
	...
	QuestStep_PepeWolves = SetQuestStatus (Quest_PepeWolves, LOG_RUNNING, QuestStep_PepeWolves);
	SetNoteEntry (Quest_PepeWolves, LOG_MISSION, "Owcom Pepe zagra¿aj¹ kr¹¿¹ce po okolicy stada wilków. Powinienem siê nimi zaj¹æ.");
};
...
{
	if (QuestStep_PepeWolves == LOG_RUNNING)
	{
		return true;
	};
};
...
func void DIA_Pepe_KilledWolves_Info()
{
	...
	&& (Npc_IsDead(PEPES_YWolf4))
	{
		...
		QuestStep_PepeWolves = SetQuestStatus (Quest_PepeWolves, LOG_SUCCESS, QuestStep_PepeWolves);
...
{
	if (QuestStep_PepeWolves == LOG_SUCCESS)
	&& (!Npc_IsDead(Bullco))
	&& (Kapitel <= 9)
	{
		return true;
...
func void DIA_Pepe_Bullco_Info()
{
	...
	QuestStep_KickBullco = SetQuestStatus (Quest_KickBullco, LOG_RUNNING, QuestStep_KickBullco);
	SetNoteEntry (Quest_KickBullco, LOG_MISSION, "Pepe jest niezadowolony, ¿e Bullko poœwiêca jego owcom tak ma³o uwagi, i szuka kogoœ, kto da³by mu nauczkê.");
};
...
{
	if (QuestStep_KickBullco == LOG_RUNNING)
	{
		if (Bullco.aivar[AIV_DefeatedByPlayer] != DBP_NONE)
		|| (DJG_Bullco.aivar[AIV_DefeatedByPlayer] != DBP_NONE)
		{
			return true;
		};
	};
...
func void DIA_Pepe_BullcoDefeated_Info()
{
	...
	QuestStep_KickBullco = SetQuestStatus (Quest_KickBullco, LOG_SUCCESS, QuestStep_KickBullco);
