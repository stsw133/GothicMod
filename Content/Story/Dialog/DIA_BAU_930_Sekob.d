...
{
	if (QuestStep_TorlofRent == LOG_RUNNING)
	{
		return true;
	};
};
...
func void DIA_Sekob_ZAHLPACHT_Info()
{
	...
	self.aivar[AIV_DefeatedByPlayer] = DBP_NONE; //WICHTIG
};
...
func void DIA_Sekob_InformOnar_Info()
{
	...
	QuestStep_SekobOnar = LOG_RUNNING;
	AI_StopProcessInfos(self);
};
...
func void DIA_Sekob_Defeated_Info()
{
	...
	AI_Output (other, self, "DIA_Sekob_Defeated_15_02"); //Zap³aæ wiêc zaleg³y czynsz.
...
{
	if (QuestStep_SekobOnar == LOG_RUNNING)
...
func void DIA_Sekob_Again_Info()
{
	...
	if (Npc_KnowsInfo(other,DIA_Onar_WegenSekob))
	{
...
{
	QuestStep_SekobOnar = LOG_SUCCESS;
...
func void DIA_Sekob_DMT_Info()
{
	...
	QuestStep_SekobDMT = SetQuestStatus (Quest_SekobDMT, LOG_RUNNING, QuestStep_SekobDMT);
	SetNoteEntry (Quest_SekobDMT, LOG_MISSION, "Czarni magowie wygnali Sekoba z domu. Sekob prosi mnie, abym siê ich pozby³.");
};
...
func void DIA_Sekob_DMTWEG_Info()
{
	...
	QuestStep_SekobDMT = SetQuestStatus (Quest_SekobDMT, LOG_SUCCESS, QuestStep_SekobDMT);
...
func void DIA_Sekob_PERM_Info()
{
	...
	else
	{
		...
		if (QuestStep_SekobRosi == LOG_FAILED)
		{
			AI_Output (self, other, "DIA_Sekob_PERM_01_01"); //Wynoœ siê.
		}
		else if (QuestStep_SekobRosi == LOG_SUCCESS)
		{
		...
		&& (QuestStep_SekobRosi != LOG_SUCCESS)
		{
			...
			QuestStep_SekobRosi = SetQuestStatus (Quest_SekobRosi, LOG_RUNNING, QuestStep_SekobRosi);
			SetNoteEntry (Quest_SekobRosi, LOG_MISSION, "¯ona Sekoba, Rosi, zaginê³a. Sekob prosi o jej odnalezienie.");
		}
		else
		{
...
func int DIA_Sekob_ROSIBACKATSEKOB_Condition()
{
	...
	&& (QuestStep_SekobRosi == LOG_RUNNING)
	{
		return true;
	};
};
...
func void DIA_Sekob_ROSIBACKATSEKOB_Info()
{
	...
	QuestStep_SekobRosi = SetQuestStatus (Quest_SekobRosi, LOG_SUCCESS, QuestStep_SekobRosi);
...
{
	if (QuestStep_SekobRosi == LOG_OBSOLETE)
	&& (hero.guild != GIL_KDF)
	{
		return true;
	};
...
func void DIA_Sekob_ROSINEVERBACK_Info()
{
	...
	QuestStep_SekobRosi = SetQuestStatus (Quest_SekobRosi, LOG_FAILED, QuestStep_SekobRosi);
	B_GivePlayerXP(XP_BONUS_1);
};
