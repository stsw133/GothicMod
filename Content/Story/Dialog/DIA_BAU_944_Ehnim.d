...
{
	if (Npc_KnowsInfo(other,DIA_Egill_STREIT4))
	&& (Hlp_IsValidNpc(Egill))
	&& (!C_NpcIsDown(Egill))
...
func void DIA_Ehnim_MoleRatFett_was()
{
	...
	QuestStep_VinoWinery = SetQuestStatus (Quest_VinoWinery, LOG_RUNNING, QuestStep_VinoWinery);
	SetNoteEntry (Quest_VinoWinery, LOG_MISSION, "Ehnim powiedzia³ mi, ¿e Vino ma sekretn¹ gorzelniê w lesie w pobli¿u farmy Akila. Jednak mechanizm otwieraj¹cy drzwi zaci¹³ siê, a do jego uruchomienia potrzebny jest ³ój kretoszczura.");
...
{
	if (Npc_KnowsInfo(other,DIA_Ehnim_MoleRatFett))
	&& (Npc_IsInState(self,ZS_Talk))
	{
		return true;
	};
