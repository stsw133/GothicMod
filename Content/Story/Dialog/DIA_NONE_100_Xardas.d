...
func void DIA_Xardas_TODO_Info()
{
	...
	QuestStep_InnosEye = SetQuestStatus (Quest_InnosEye, LOG_RUNNING, QuestStep_InnosEye);
...
func int DIA_Addon_Xardas_Portal_Condition()
{
	...
	&& (!Npc_KnowsInfo(other,DIA_Addon_Saturas_BeliarWeapGeben))
	{
		return true;
	};
};
...
func void DIA_Xardas_ABOUTLESTER_Info()
{
	...
	B_GiveInvItems (self, other, ItRi_Resist_01, 1);
	B_GivePlayerXP(XP_BONUS(50));
};
...
func void DIA_Xardas_DMTSINDDA_Info()
{
	...
	SetNoteEntry (Quest_InnosEye, LOG_MISSION, "Wróg dowiedzia³ siê, ¿e szukam Oka Innosa. Muszê je jak najszybciej znaleŸæ, zanim bêdzie za póŸno.");
...
func void DIA_Xardas_DMTSINDDA_DMT()
{
	...
	{
		QuestStep_Dementors = SetQuestStatus (Quest_Dementors, LOG_RUNNING, QuestStep_Dementors);
		SetNoteEntry (Quest_Dementors, LOG_MISSION, "Xardas wie o odzianych na czarno mê¿czyznach. Wygl¹da na to, ¿e tak naprawdê to Poszukiwacze poci¹gaj¹ za sznurki, i s¹ bardzo niebezpiecznymi przeciwnikami.");
	};
};
...
func void DIA_Xardas_DMTSINDDA_DMT_WhatToDo()
{
	...
	SetNoteEntry (Quest_InnosEye, LOG_MISSION, "Oko znajduje siê w klasztorze Magów Ognia. Mam nadziejê, ¿e Lord Hagen pozwoli mi siê do niego dostaæ, kiedy dostarczê mu wiadomoœæ od Lorda Garonda. Mag Ognia Pyrokar nie dopuœci mnie w pobli¿e amuletu, jeœli nie bêdê mia³ poparcia Hagena.");
};
...
func int DIA_Xardas_INNOSEYEBROKEN_Condition()
{
	...
	&& (Npc_HasItems(other,ItAm_InnosEye_Broken) || MIS_SCKnowsInnosEyeIsBroken)
	{
		return true;
	};
};
...
func void DIA_Xardas_INNOSEYEBROKEN_wasnun()
{
	...
	SetNoteEntry (Quest_InnosEye, LOG_MISSION, "Xardas nie by³ zachwycony, kiedy dowiedzia³ siê, ¿e Oko Innosa zosta³o zniszczone. Jedyn¹ nasz¹ nadziej¹ jest Vatras, Mag Wody z miasta Khorinis.");
};
...
{
	if (QuestStep_InnosEyeRitual == LOG_RUNNING)
...
func void DIA_Xardas_RITUALREQUEST_Info()
{
	...
	|| (hero.guild == GIL_PAL)
	{
		...
		SetNoteEntry (Quest_InnosEye, LOG_MISSION, "Xardas zgodzi³ siê przyjœæ do S³onecznego Krêgu na rytua³.");
		B_GivePlayerXP(XP_BONUS(300));
...
func int DIA_Xardas_WARUMNICHTJETZT_Condition()
{
	...
	&& (hero.guild == GIL_MIL || hero.guild == GIL_SLD)
	{
		return true;
	};
};
...
func int DIA_Xardas_BEREIT_Condition()
{
	...
	&& (hero.guild == GIL_DJG || hero.guild == GIL_PAL)
	{
		return true;
	};
};
...
{
	if (Npc_KnowsInfo(other,DIA_Pyrokar_XARDASVERTRAUEN))
	&& (Npc_KnowsInfo(other,DIA_Xardas_RITUALREQUEST))
	&& (Kapitel == 9)
	{
		return true;
...
func void DIA_Xardas_PYROWILLNICHT_Info()
{
	...
	SetNoteEntry (Quest_InnosEye, LOG_MISSION, "Xardas da³ mi klucz do kufra na farmie Sekoba. W œrodku znajdê ksi¹¿kê, któr¹ mam wrêczyæ Pyrokarowi.");
};
...
{
	if (QuestStep_InnosEyeRitual == LOG_SUCCESS)
	&& (Kapitel == 9)
	{
		return true;
	};
