...
{
	if (self.aivar[AIV_CanTeach] == DONE)
	&& (Npc_GetTalentSkill(other,NPC_TALENT_SMITH) < 4)
...
func void DIA_Bennet_Present_Info ()
{
	...
	Info_ClearChoices (DIA_Bennet_Present);
	}
	else if (Npc_KnowsInfo (other,DIA_Bennet_ThankYou))
	&& (hero.guild == GIL_MIL)
	|| (hero.guild == GIL_PAL)
	|| (hero.guild == GIL_KDF)
	{
...
{	
	...
	Bennet_DragonScale_Counter += Npc_HasItems(other,ItAT_Dragonscale);
	
	B_GiveInvItems (self ,other,ItMi_Gold,(Npc_HasItems(other,ItAT_Dragonscale) * 200) ); 
...
{
	if (Npc_HasItems (other,ItAm_InnosEye_Broken))
...
{
	IF (Npc_HasItems (other,ItAm_InnosEye_Broken)	>=1)
	&& (MIS_SCKnowsInnosEyeIsBroken  == true)
	&& (MIS_REscueBennet == LOG_SUCCESS)
	&& (MIS_Bennet_InnosEyeRepairedSetting   != LOG_SUCCESS)
	{
...
func void DIA_Bennet_GiveInnosEye_Info ()
{
	...
	Npc_RemoveInvItems	(other,	ItAm_InnosEye_Broken,1);
	AI_PrintScreen ("Odda³eœ Oko Innosa", -1, YPOS_ItemGiven, FONT_ScreenSmall, 2);
...
func void DIA_Bennet_GetInnosEye_Info ()
{
	...
	if (Bennet_RepairDay < Wld_GetDay())
	{
		...
		CreateInvItems (other,ItAm_InnosEye_Broken,1);
		AI_PrintScreen ("Otrzyma³eœ Oko Innosa", -1, YPOS_ItemGiven, FONT_ScreenSmall, 2);
...
func void DIA_Bennet_EierBringen_Info ()
{
	...
	if (DragonEggCount == 1)
		...
		else
		{
			...
			XP_DJG_BringDragonEggs = (DragonEggCount * XP_BONUS_3);
