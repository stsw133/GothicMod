...
{
	if (NpcObsessedByDMT_Fernando == false)
	&& (QuestStep_OldWorld == LOG_RUNNING)
	&& (Kapitel == 8)
...
FUNC VOID DIA_Fernando_Minental_Info()
{
	...
	QuestStep_FernandoNews = SetQuestStatus (Quest_FernandoNews, LOG_RUNNING, QuestStep_FernandoNews);
	SetNoteEntry (Quest_FernandoNews, LOG_MISSION, "Kupiec Fernando chce wiedzieæ, co dzieje siê z rud¹ w Górniczej Dolinie.");
};
...
func void DIA_Addon_Fernando_BanditTrader_ja()
{
	...
	Npc_RemoveInvItem (hero, ItRi_BanditTrader);
	...
	CreateInvItems (self, ItRi_Splendor_01, 1);
	B_GiveInvItems (self, other, ItRi_Splendor_01, 1);
...
FUNC VOID DIA_Fernando_Success_Info()
{
	...
	if (Fernando_ImKnast == false)
	{
		...
		QuestStep_FernandoNews = SetQuestStatus (Quest_FernandoNews, LOG_SUCCESS, QuestStep_FernandoNews);
	...
	{
		B_Say (self, other, "$NOTNOW");
		QuestStep_FernandoNews = SetQuestStatus (Quest_FernandoNews, LOG_OBSOLETE, QuestStep_FernandoNews);
		SetNoteEntry (Quest_FernandoNews, LOG_MISSION, "Fernando nie chce kolejnych informacji. A ja nie chcê kolejnej zap³aty.");
	};
};
