...
func void DIA_Harad_Arbeit_Info()
{
	...
	SetNoteEntry (Quest_CityWork, LOG_MISSION, "Jeœli zdo³am przekonaæ Harada, ¿e jestem godny zaufania, przyjmie mnie na swojego czeladnika.");
};
...
func void DIA_Harad_Taugenichts_Info()
{
	...
	QuestStep_HaradOrc = SetQuestStatus (Quest_HaradOrc, LOG_RUNNING, QuestStep_HaradOrc);
	SetNoteEntry (Quest_HaradOrc, LOG_MISSION, "Poza murami miasta zauwa¿ono orka. Kowal Harad chce, abym go zabi³. Kiedy wykonam zadanie, mam przynieœæ jego broñ.");
};
...
{
	if (QuestStep_HaradOrc == LOG_RUNNING)
...
func void DIA_Harad_OrcRunning_TooHard()
{
	...
	if (QuestStep_HakonBandits != LOG_RUNNING)
	{
		SetNoteEntry (Quest_CityWork, LOG_MISSION, "Harad powiedzia³ mi, ¿e bandyci napadli poza murami miasta na kupca Hakona. Mo¿e od niego dowiem siê, gdzie mogê ich znaleŸæ.");
	}
	else
	{
		SetNoteEntry (Quest_CityWork, LOG_MISSION, "Harad powiedzia³ mi, ¿e bandyci napadli poza murami miasta na kupca Hakona. Jeœli zrobiê z nimi porz¹dek, udowodniê swoj¹ wartoœæ.");
	};
...
func void DIA_Harad_OrcSuccess_Info ()
{
	...
	QuestStep_HaradOrc = SetQuestStatus (Quest_HaradOrc, LOG_SUCCESS, QuestStep_HaradOrc);
	SetNoteEntry (Quest_CityWork, LOG_MISSION, "Harad przyjmie mnie na swojego czeladnika, jeœli zdobêdê poparcie pozosta³ych mistrzów.");
...
func void DIA_Harad_LEHRLING_Info()
{
	...
	if (QuestStep_HaradOrc == LOG_SUCCESS)
	|| ((QuestStep_HakonBandits == LOG_SUCCESS) && (Harad_HakonMission == true))
	{
		// ------ Harad ------
		if (QuestStep_HaradOrc == LOG_SUCCESS)
		{
			...
			if (QuestStep_HakonBandits == LOG_SUCCESS)
			{
		...
		{
			if (QuestStep_ThorbenBless == LOG_SUCCESS)
			{
		...
		if (Bosper.aivar[AIV_TalkedToPlayer] == true)
		{
			...
			if (QuestStep_BosperBow == LOG_SUCCESS)
			|| (QuestStep_BosperFurs == LOG_SUCCESS)
			{
		...
		if (Matteo.aivar[AIV_TalkedToPlayer] == true)
		{
			...
			else if (QuestStep_MatteoGritta == LOG_RUNNING)
			{
...
func void DIA_Harad_LEHRLING_OK()
{
	...
	QuestStep_CityWork = SetQuestStatus (Quest_CityWork, LOG_SUCCESS, QuestStep_CityWork);
	SetNoteEntry (Quest_CityWork, LOG_MISSION, "Harad przyj¹³ mnie na swojego czeladnika. Droga do górnego miasta stoi przede mn¹ otworem.");
	SetNoteEntry (Note_Special, LOG_NOTE, "Harad dobrze mi zap³aci za wykut¹ przeze mnie broñ.");
	SetNoteEntry (Note_Teachers, LOG_NOTE, "Harad mo¿e mnie nauczyæ rzemios³a kowalskiego. Mo¿e równie¿ zwiêkszyæ moj¹ si³ê.");
	B_GivePlayerXP(XP_BONUS_3);
	
	CreateInvItems (other, ItAr_Smith, 1);
	AI_EquipArmor (other, ItAr_Smith);
...
func void DIA_Harad_SellBlades_Info()
{
	...
	else //(gesamt >= 1) - die verbeliebenden Klingen verkaufen ------
	{
		...
		var string concatText; concatText = ConcatStrings(IntToString(gesamt),PRINT_ItemsGegeben);
		AI_PrintScreen (concatText, -1, YPOS_ItemGiven, FONT_ScreenSmall, 2);
		
		AI_Output (self, other, "DIA_Harad_SellBlades_12_03"); //Dobrze. Oto twoja zap³ata.
		var int lohn;
		lohn = (anzahl_common * ItMw_1H_Common_01.value)
			 + (anzahl_schwert1 * ItMw_Schwert1.value)
			 + (anzahl_schwert4 * ItMw_Schwert4.value)
			 + (anzahl_rubinklinge * ItMw_Rubinklinge.value)
			 + (anzahl_elbastardo * ItMw_ElBastardo.value);
