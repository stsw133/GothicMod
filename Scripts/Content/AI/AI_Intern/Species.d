...
///	SPECIES
///******************************************************************************************
/// WARNING: Turn-Speed is doubled in fight mode (monsters are always in fight mode)!
...
// LIVESTOCK (SHEEP & GOAT)
	...
	SWIM_TIME		 		[GIL_LIVESTOCK] = TIME_INFINITE;
	DIVE_TIME		 		[GIL_LIVESTOCK] = 0;
	WATER_DEPTH_KNEE 		[GIL_LIVESTOCK] = 25;
	WATER_DEPTH_CHEST		[GIL_LIVESTOCK] = 50;

	JUMPLOW_HEIGHT 			[GIL_LIVESTOCK] = 105;
	JUMPMID_HEIGHT 			[GIL_LIVESTOCK] = 200;

	SLIDE_ANGLE		 		[GIL_LIVESTOCK] = 50;
	DISABLE_AUTOROLL 		[GIL_LIVESTOCK] = 1;
	SURFACE_ALIGN	 		[GIL_LIVESTOCK] = 1;
	TURN_SPEED				[GIL_LIVESTOCK] = 120; //!!! NICHT kleiner als 100, sonst wird Turn-Ani nicht gespielt (Bug)

	// Fallhöhe / Fallschaden
	FALLDOWN_HEIGHT			[GIL_LIVESTOCK] = 400;
	FALLDOWN_DAMAGE			[GIL_LIVESTOCK] = 10;

	// Kampf
	FIGHT_RANGE_BASE		[GIL_LIVESTOCK] = 130;
	FIGHT_RANGE_FIST		[GIL_LIVESTOCK] = 95;
	FIGHT_RANGE_G			[GIL_LIVESTOCK] = 95;

	// Bluteffekte by KaiRo
	BLOOD_EMITTER			[GIL_LIVESTOCK] = "BFX_Preset1";
	BLOOD_TEXTURE			[GIL_LIVESTOCK] = "BQM_Scavenger2.tga";
	BLOOD_FLOW				[GIL_LIVESTOCK] = 1;
	BLOOD_AMOUNT			[GIL_LIVESTOCK] = 10;
	BLOOD_MAX_DISTANCE		[GIL_LIVESTOCK] = 100;
...
// SUMMONED GOLEM
	...
	SWIM_TIME		 		[GIL_SUMMONED_GOLEM] = SWIM_TIME		 	[GIL_GOLEM];
	DIVE_TIME		 		[GIL_SUMMONED_GOLEM] = DIVE_TIME		 	[GIL_GOLEM];
	WATER_DEPTH_KNEE 		[GIL_SUMMONED_GOLEM] = WATER_DEPTH_KNEE 	[GIL_GOLEM];
	WATER_DEPTH_CHEST		[GIL_SUMMONED_GOLEM] = WATER_DEPTH_CHEST	[GIL_GOLEM];

	SLIDE_ANGLE		 		[GIL_SUMMONED_GOLEM] = SLIDE_ANGLE		 	[GIL_GOLEM];    
	DISABLE_AUTOROLL 		[GIL_SUMMONED_GOLEM] = DISABLE_AUTOROLL 	[GIL_GOLEM];
	SURFACE_ALIGN	 		[GIL_SUMMONED_GOLEM] = SURFACE_ALIGN	 	[GIL_GOLEM];
	TURN_SPEED				[GIL_SUMMONED_GOLEM] = TURN_SPEED			[GIL_GOLEM];    

	// Fallhöhe / Fallschaden                      // Fallhöhe / Fallschaden                
	FALLDOWN_HEIGHT			[GIL_SUMMONED_GOLEM] = FALLDOWN_HEIGHT		[GIL_GOLEM];    
	FALLDOWN_DAMAGE			[GIL_SUMMONED_GOLEM] = FALLDOWN_DAMAGE		[GIL_GOLEM];    

	// Kampf                                       // Kampf                                 
	FIGHT_RANGE_BASE		[GIL_SUMMONED_GOLEM] = FIGHT_RANGE_BASE		[GIL_GOLEM];
	FIGHT_RANGE_FIST		[GIL_SUMMONED_GOLEM] = FIGHT_RANGE_FIST		[GIL_GOLEM];
    FIGHT_RANGE_G			[GIL_SUMMONED_GOLEM] = FIGHT_RANGE_G		[GIL_GOLEM];
...
// SwampGolem
	...
	SWIM_TIME		 		[GIL_SWAMPGOLEM] = SWIM_TIME		 	[GIL_GOLEM];        
	DIVE_TIME		 		[GIL_SWAMPGOLEM] = DIVE_TIME		 	[GIL_GOLEM];        
	WATER_DEPTH_KNEE 		[GIL_SWAMPGOLEM] = WATER_DEPTH_KNEE 	[GIL_GOLEM];        
	WATER_DEPTH_CHEST		[GIL_SWAMPGOLEM] = WATER_DEPTH_CHEST	[GIL_GOLEM];        

	STEP_HEIGHT		 		[GIL_SWAMPGOLEM] = STEP_HEIGHT		 	[GIL_GOLEM];    
	JUMPLOW_HEIGHT	 		[GIL_SWAMPGOLEM] = JUMPLOW_HEIGHT	 	[GIL_GOLEM];
	JUMPMID_HEIGHT	 		[GIL_SWAMPGOLEM] = JUMPMID_HEIGHT	 	[GIL_GOLEM];
	JUMPUP_HEIGHT 			[GIL_SWAMPGOLEM] = JUMPUP_HEIGHT 		[GIL_GOLEM];

	SLIDE_ANGLE		 		[GIL_SWAMPGOLEM] = SLIDE_ANGLE		 	[GIL_GOLEM];    
	TURN_SPEED				[GIL_SWAMPGOLEM] = TURN_SPEED			[GIL_GOLEM];

	// Kletterkanten - Winkel                 // Kletterkanten - Winkel              
	CLIMB_HEADING_ANGLE		[GIL_SWAMPGOLEM] = CLIMB_HEADING_ANGLE	[GIL_GOLEM];    
	CLIMB_HORIZ_ANGLE		[GIL_SWAMPGOLEM] = CLIMB_HORIZ_ANGLE	[GIL_GOLEM];        
	CLIMB_GROUND_ANGLE  	[GIL_SWAMPGOLEM] = CLIMB_GROUND_ANGLE  	[GIL_GOLEM];        

	// Fallhöhe / Fallschaden                 // Fallhöhe / Fallschaden              
	FALLDOWN_HEIGHT			[GIL_SWAMPGOLEM] = FALLDOWN_HEIGHT		[GIL_GOLEM];     
	FALLDOWN_DAMAGE			[GIL_SWAMPGOLEM] = FALLDOWN_DAMAGE		[GIL_GOLEM];    

	// Kampf                                  // Kampf                               
	FIGHT_RANGE_BASE		[GIL_SWAMPGOLEM] = FIGHT_RANGE_BASE		[GIL_GOLEM];        
	FIGHT_RANGE_FIST		[GIL_SWAMPGOLEM] = FIGHT_RANGE_FIST		[GIL_GOLEM];       
	FIGHT_RANGE_G			[GIL_SWAMPGOLEM] = FIGHT_RANGE_G		[GIL_GOLEM]; 

// Steinpuma

	SWIM_TIME		 		[GIL_GARGOYLE] = SWIM_TIME		 		[GIL_HARPY];        
	DIVE_TIME		 		[GIL_GARGOYLE] = DIVE_TIME		 		[GIL_HARPY];        
	WATER_DEPTH_KNEE 		[GIL_GARGOYLE] = WATER_DEPTH_KNEE 		[GIL_HARPY];        
	WATER_DEPTH_CHEST		[GIL_GARGOYLE] = WATER_DEPTH_CHEST		[GIL_HARPY];        

	STEP_HEIGHT		 		[GIL_GARGOYLE] = STEP_HEIGHT		 	[GIL_HARPY];    
	JUMPLOW_HEIGHT	 		[GIL_GARGOYLE] = JUMPLOW_HEIGHT	 		[GIL_HARPY];
	JUMPMID_HEIGHT	 		[GIL_GARGOYLE] = JUMPMID_HEIGHT	 		[GIL_HARPY];
	JUMPUP_HEIGHT 			[GIL_GARGOYLE] = JUMPUP_HEIGHT 			[GIL_HARPY];

	SLIDE_ANGLE		 		[GIL_GARGOYLE] = SLIDE_ANGLE		 	[GIL_HARPY];    
	TURN_SPEED				[GIL_GARGOYLE] = TURN_SPEED				[GIL_HARPY];

	// Kletterkanten - Winkel                 // Kletterkanten - Winkel              
	CLIMB_HEADING_ANGLE		[GIL_GARGOYLE] = CLIMB_HEADING_ANGLE	[GIL_HARPY];    
	CLIMB_HORIZ_ANGLE		[GIL_GARGOYLE] = CLIMB_HORIZ_ANGLE		[GIL_HARPY];        
	CLIMB_GROUND_ANGLE  	[GIL_GARGOYLE] = CLIMB_GROUND_ANGLE  	[GIL_HARPY];        

	// Fallhöhe / Fallschaden                 // Fallhöhe / Fallschaden              
	FALLDOWN_HEIGHT			[GIL_GARGOYLE] = FALLDOWN_HEIGHT		[GIL_HARPY];     
	FALLDOWN_DAMAGE			[GIL_GARGOYLE] = FALLDOWN_DAMAGE		[GIL_HARPY];    

	// Kampf                                  // Kampf                               
	FIGHT_RANGE_BASE		[GIL_GARGOYLE] = FIGHT_RANGE_BASE		[GIL_HARPY];        
	FIGHT_RANGE_FIST		[GIL_GARGOYLE] = FIGHT_RANGE_FIST		[GIL_HARPY];       
	FIGHT_RANGE_G			[GIL_GARGOYLE] = FIGHT_RANGE_G			[GIL_HARPY]; 
	
	//Blut
	BLOOD_EMITTER			[GIL_GARGOYLE] = "BFX_Golem";
	BLOOD_TEXTURE			[GIL_GARGOYLE] = "BQM_Golem.tga";
	BLOOD_FLOW				[GIL_GARGOYLE] = 0;
	BLOOD_AMOUNT			[GIL_GARGOYLE] = 5;
	BLOOD_MAX_DISTANCE		[GIL_GARGOYLE] = 60;
	
// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
// OGRE

	SWIM_TIME		 		[GIL_OGRE] = SWIM_TIME		 		[GIL_ORC];        
	DIVE_TIME		 		[GIL_OGRE] = DIVE_TIME		 		[GIL_ORC];        
	WATER_DEPTH_KNEE 		[GIL_OGRE] = WATER_DEPTH_KNEE 		[GIL_ORC];        
	WATER_DEPTH_CHEST		[GIL_OGRE] = WATER_DEPTH_CHEST		[GIL_ORC];        

	STEP_HEIGHT		 		[GIL_OGRE] = STEP_HEIGHT		 	[GIL_ORC];    
	JUMPLOW_HEIGHT	 		[GIL_OGRE] = JUMPLOW_HEIGHT	 		[GIL_ORC];
	JUMPMID_HEIGHT	 		[GIL_OGRE] = JUMPMID_HEIGHT	 		[GIL_ORC];
	JUMPUP_HEIGHT 			[GIL_OGRE] = JUMPUP_HEIGHT 			[GIL_ORC];

	SLIDE_ANGLE		 		[GIL_OGRE] = SLIDE_ANGLE		 	[GIL_ORC];    
	TURN_SPEED				[GIL_OGRE] = TURN_SPEED				[GIL_ORC];

	// Kletterkanten - Winkel                 // Kletterkanten - Winkel              
	CLIMB_HEADING_ANGLE		[GIL_OGRE] = CLIMB_HEADING_ANGLE	[GIL_ORC];    
	CLIMB_HORIZ_ANGLE		[GIL_OGRE] = CLIMB_HORIZ_ANGLE		[GIL_ORC];        
	CLIMB_GROUND_ANGLE  	[GIL_OGRE] = CLIMB_GROUND_ANGLE  	[GIL_ORC];        

	// Fallhöhe / Fallschaden                 // Fallhöhe / Fallschaden              
	FALLDOWN_HEIGHT			[GIL_OGRE] = FALLDOWN_HEIGHT		[GIL_ORC];     
	FALLDOWN_DAMAGE			[GIL_OGRE] = FALLDOWN_DAMAGE		[GIL_ORC];    

	// Kampf                                  // Kampf                               
	FIGHT_RANGE_BASE		[GIL_OGRE] = FIGHT_RANGE_BASE		[GIL_ORC];        
	FIGHT_RANGE_FIST		[GIL_OGRE] = FIGHT_RANGE_FIST		[GIL_ORC];       
	FIGHT_RANGE_G			[GIL_OGRE] = FIGHT_RANGE_G			[GIL_ORC]; 

// Steinwächter

	SWIM_TIME		 		[GIL_STONEGUARDIAN] = SWIM_TIME		 		[GIL_GOLEM];        
	DIVE_TIME		 		[GIL_STONEGUARDIAN] = DIVE_TIME		 		[GIL_GOLEM];        
	WATER_DEPTH_KNEE 		[GIL_STONEGUARDIAN] = WATER_DEPTH_KNEE 		[GIL_GOLEM];        
	WATER_DEPTH_CHEST		[GIL_STONEGUARDIAN] = WATER_DEPTH_CHEST		[GIL_GOLEM];        
	...
	SWIM_TIME		 		[GIL_SUMMONEDGUARDIAN] = SWIM_TIME		 		[GIL_GOLEM];        
	DIVE_TIME		 		[GIL_SUMMONEDGUARDIAN] = DIVE_TIME		 		[GIL_GOLEM];        
	WATER_DEPTH_KNEE 		[GIL_SUMMONEDGUARDIAN] = WATER_DEPTH_KNEE 		[GIL_GOLEM];        
	WATER_DEPTH_CHEST		[GIL_SUMMONEDGUARDIAN] = WATER_DEPTH_CHEST		[GIL_GOLEM];        
...
//	SWIM_TIME		 		[GIL_NEUTRAL] = SWIM_TIME		 		[GIL_WOLF];
//	DIVE_TIME		 		[GIL_NEUTRAL] = DIVE_TIME		 		[GIL_WOLF];
//	WATER_DEPTH_KNEE 		[GIL_NEUTRAL] = WATER_DEPTH_KNEE 		[GIL_WOLF];
//	WATER_DEPTH_CHEST		[GIL_NEUTRAL] = WATER_DEPTH_CHEST		[GIL_WOLF];
//	
//	STEP_HEIGHT		 		[GIL_NEUTRAL] = STEP_HEIGHT		 		[GIL_WOLF];
//	JUMPLOW_HEIGHT	 		[GIL_NEUTRAL] = JUMPLOW_HEIGHT	 		[GIL_WOLF];
//	JUMPMID_HEIGHT	 		[GIL_NEUTRAL] = JUMPMID_HEIGHT	 		[GIL_WOLF];
//	
//	SLIDE_ANGLE		 		[GIL_NEUTRAL] = SLIDE_ANGLE		 		[GIL_WOLF];
//	DISABLE_AUTOROLL 		[GIL_NEUTRAL] = DISABLE_AUTOROLL 		[GIL_WOLF];
//	SURFACE_ALIGN	 		[GIL_NEUTRAL] = SURFACE_ALIGN	 		[GIL_WOLF];
//	TURN_SPEED				[GIL_NEUTRAL] = TURN_SPEED				[GIL_WOLF];
//	
//	FALLDOWN_HEIGHT			[GIL_NEUTRAL] = FALLDOWN_HEIGHT			[GIL_WOLF];
//	FALLDOWN_DAMAGE			[GIL_NEUTRAL] = FALLDOWN_DAMAGE			[GIL_WOLF];
//	
//	FIGHT_RANGE_BASE		[GIL_NEUTRAL] = FIGHT_RANGE_BASE		[GIL_WOLF];
//	FIGHT_RANGE_FIST		[GIL_NEUTRAL] = FIGHT_RANGE_FIST		[GIL_WOLF];
//	FIGHT_RANGE_G			[GIL_NEUTRAL] = FIGHT_RANGE_G			[GIL_WOLF];
//	
//	BLOOD_EMITTER			[GIL_NEUTRAL] = "BFX_Preset1";     	
//	BLOOD_TEXTURE			[GIL_NEUTRAL] = "BQM_Scaveng2.tga";
//	BLOOD_FLOW				[GIL_NEUTRAL] = BLOOD_FLOW				[GIL_WOLF];
//	BLOOD_AMOUNT			[GIL_NEUTRAL] = BLOOD_AMOUNT			[GIL_WOLF];
//	BLOOD_MAX_DISTANCE		[GIL_NEUTRAL] = BLOOD_MAX_DISTANCE		[GIL_WOLF];
	
// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
// WILD
	
	SWIM_TIME		 		[GIL_WILD] = SWIM_TIME		 			[GIL_WOLF];
	DIVE_TIME		 		[GIL_WILD] = DIVE_TIME		 			[GIL_WOLF];
	WATER_DEPTH_KNEE 		[GIL_WILD] = WATER_DEPTH_KNEE 			[GIL_WOLF];
	WATER_DEPTH_CHEST		[GIL_WILD] = WATER_DEPTH_CHEST			[GIL_WOLF];
	
	STEP_HEIGHT		 		[GIL_WILD] = STEP_HEIGHT		 		[GIL_WOLF];
	JUMPLOW_HEIGHT	 		[GIL_WILD] = JUMPLOW_HEIGHT	 			[GIL_WOLF];
	JUMPMID_HEIGHT	 		[GIL_WILD] = JUMPMID_HEIGHT	 			[GIL_WOLF];
	
	SLIDE_ANGLE		 		[GIL_WILD] = SLIDE_ANGLE		 		[GIL_WOLF];
	DISABLE_AUTOROLL 		[GIL_WILD] = DISABLE_AUTOROLL 			[GIL_WOLF];
	SURFACE_ALIGN	 		[GIL_WILD] = SURFACE_ALIGN	 			[GIL_WOLF];
	TURN_SPEED				[GIL_WILD] = TURN_SPEED					[GIL_WOLF];
	
	FALLDOWN_HEIGHT			[GIL_WILD] = FALLDOWN_HEIGHT			[GIL_WOLF];
	FALLDOWN_DAMAGE			[GIL_WILD] = FALLDOWN_DAMAGE			[GIL_WOLF];
	
	FIGHT_RANGE_BASE		[GIL_WILD] = FIGHT_RANGE_BASE			[GIL_WOLF];
	FIGHT_RANGE_FIST		[GIL_WILD] = FIGHT_RANGE_FIST			[GIL_WOLF];
	FIGHT_RANGE_G			[GIL_WILD] = FIGHT_RANGE_G				[GIL_WOLF];
	
	BLOOD_EMITTER			[GIL_WILD] = "BFX_Preset1";     	
	BLOOD_TEXTURE			[GIL_WILD] = "BQM_Scaveng2.tga";
	BLOOD_FLOW				[GIL_WILD] = BLOOD_FLOW					[GIL_WOLF];
	BLOOD_AMOUNT			[GIL_WILD] = BLOOD_AMOUNT				[GIL_WOLF];
	BLOOD_MAX_DISTANCE		[GIL_WILD] = BLOOD_MAX_DISTANCE			[GIL_WOLF];
	
// +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
// SUMMONED
	
	SWIM_TIME				[GIL_SUMMONED] = SWIM_TIME				[GIL_SKELETON];
	DIVE_TIME		 		[GIL_SUMMONED] = DIVE_TIME		 		[GIL_SKELETON];
	WATER_DEPTH_KNEE 		[GIL_SUMMONED] = WATER_DEPTH_KNEE 		[GIL_SKELETON];
	WATER_DEPTH_CHEST		[GIL_SUMMONED] = WATER_DEPTH_CHEST		[GIL_SKELETON];
	
	STEP_HEIGHT		 		[GIL_SUMMONED] = STEP_HEIGHT		 	[GIL_SKELETON];
	JUMPLOW_HEIGHT	 		[GIL_SUMMONED] = JUMPLOW_HEIGHT	 		[GIL_SKELETON];
	JUMPMID_HEIGHT	 		[GIL_SUMMONED] = JUMPMID_HEIGHT	 		[GIL_SKELETON];
	JUMPUP_HEIGHT 			[GIL_SUMMONED] = JUMPUP_HEIGHT 			[GIL_SKELETON];
	
	SLIDE_ANGLE		 		[GIL_SUMMONED] = SLIDE_ANGLE		 	[GIL_SKELETON];
	TURN_SPEED				[GIL_SUMMONED] = TURN_SPEED				[GIL_SKELETON];
	
	CLIMB_HEADING_ANGLE		[GIL_SUMMONED] = CLIMB_HEADING_ANGLE	[GIL_SKELETON];
	CLIMB_HORIZ_ANGLE		[GIL_SUMMONED] = CLIMB_HORIZ_ANGLE		[GIL_SKELETON];
	CLIMB_GROUND_ANGLE  	[GIL_SUMMONED] = CLIMB_GROUND_ANGLE  	[GIL_SKELETON];
	
	FALLDOWN_HEIGHT			[GIL_SUMMONED] = FALLDOWN_HEIGHT		[GIL_SKELETON];
	FALLDOWN_DAMAGE			[GIL_SUMMONED] = FALLDOWN_DAMAGE		[GIL_SKELETON];
	
	FIGHT_RANGE_BASE		[GIL_SUMMONED] = FIGHT_RANGE_BASE		[GIL_SKELETON];
	FIGHT_RANGE_FIST		[GIL_SUMMONED] = FIGHT_RANGE_FIST		[GIL_SKELETON];
	FIGHT_RANGE_G			[GIL_SUMMONED] = FIGHT_RANGE_G			[GIL_SKELETON];
	
	BLOOD_EMITTER			[GIL_SUMMONED] = "BFX_Skeleton";
