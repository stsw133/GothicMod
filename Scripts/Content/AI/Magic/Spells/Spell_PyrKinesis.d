...
const int SPL_Cost_PyrKinesis			=	80;
const int SPL_Damage_PyrKinesis			=	100;

///******************************************************************************************
instance Spell_PyrKinesis (C_Spell_Proto)
{
	time_per_mana						=	0;
	damage_per_level					=	SPL_Damage_PyrKinesis;
	damageType							=	DAM_MAGIC;
};

func int Spell_Logic_PyrKinesis (var int manaInvested)
{
	if (Npc_GetActiveSpellIsScroll(self) && (self.attribute[ATR_MANA] >= SPL_Cost_PyrKinesis/SPL_Cost_Scroll))
	|| (self.attribute[ATR_MANA] >= SPL_Cost_PyrKinesis)
	{
		return SPL_SENDCAST;
	};
	return SPL_SENDSTOP;
...
func void Spell_Cast_PyrKinesis()
{
	if (Npc_GetActiveSpellIsScroll(self))
	{
		self.attribute[ATR_MANA] -= SPL_Cost_PyrKinesis/SPL_Cost_Scroll;
	}
	else
	{
		self.attribute[ATR_MANA] -= SPL_Cost_PyrKinesis;
