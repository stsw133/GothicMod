...
{
	var int items; items = Npc_HasItems(self, ItSe_AmmoBox);
	Npc_RemoveInvItems (self, ItSe_AmmoBox, items);
...
{
	var int items; items = Npc_HasItems(self, ItSe_HAmmoBox);
	Npc_RemoveInvItems (self, ItSe_HAmmoBox, items);
	B_PlayerFindItem (ItRw_HAmmo, items*6 + 6);
};
