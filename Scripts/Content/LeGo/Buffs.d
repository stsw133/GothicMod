...
const int BUFF_Y = 100;			//7000
const int BUFF_HEIGHT = 300;	//500
...
func void _Bufflist_UpdateDurationFade()
{
	...
	repeat(k, arr.numInArray);
		...
		else if (new_alpha > 255) { new_alpha = 255; };
...
{
	// This is called every tick and is responsible for deleting the object 
...
func int Buff_Apply (var C_Npc npc, var int buff)
{
	...
	if (!b.tickMS)
	{
		b.tickMS = b.durationMS+1; /* Increase by one so tickCount is zero */
	};
