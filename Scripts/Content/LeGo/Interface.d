...
func int Print_ExtPrcnt(var int x, var int y, var string text, var string font, var int color, var int time)
{
	var int preX; preX = (PS_VMax - Print_ToVirtual(Print_GetStringWidth(text, font), PS_X)) * x / 100;
	if (x == -1) { preX = (PS_VMax - Print_ToVirtual(Print_GetStringWidth(text, font), PS_X)) / 2;	};
	var int preY; preY = (PS_VMax - Print_ToVirtual(Print_GetFontHeight(font), PS_Y)) * y / 100;
	if (y == -1) { preY = (PS_VMax - Print_ToVirtual(Print_GetFontHeight(font), PS_Y)) / 2;	};
	Print_Ext(preX, preY, text, font, color, time);
};
