...
//	};
	...
	if (Npc_GetLifeStealPoints(other) > 0)
	{
		var C_Item wpn; wpn = Npc_GetEquippedMeleeWeapon(other);
		var int dmg; dmg = other.attribute[ATR_STRENGTH] + wpn.damageTotal - self.protection[PROT_EDGE];
		
		if (dmg > 0)
		{
			if (other.guild > GIL_SEPERATOR_HUM)
			{
				Npc_ChangeAttribute (other, ATR_HITPOINTS, (Npc_GetLifeStealPoints(other) * dmg)/100);
			}
			else if (Npc_IsInFightMode(other, FMODE_MELEE))
			{
				if (dmg > Npc_GetLifeStealPoints(other))
				{
					Npc_ChangeAttribute(other, ATR_HITPOINTS, Npc_GetLifeStealPoints(other));
				}
				else
				{
					Npc_ChangeAttribute(other, ATR_HITPOINTS, dmg);
				};
			};
		};
	};
