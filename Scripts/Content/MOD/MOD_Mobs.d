...
func void oCSpawnManager_DeleteNpc (var int slfPtr)
{
	...
	if (CALL_Begin (call))
	{
		...
		CALL__thiscall(_@(MEM_Game.spawnman), 7837328);
		call = CALL_End();
	};
};
...
func int Hlp_Is_oCNpcInventory (var int ptr)
{
	...
	return (MEM_ReadInt(ptr) == 8636180);
};
...
func void _hook_oCItemContainer_OpenPassive()
{
	...
	if (!Hlp_Is_oCNpc(npcInventory.owner))
	{
		return;
	};
	
	var oCNpc slf; slf = _^(npcInventory.owner);
	if (!NPC_IsPlayer(slf))
	{
		return;
	};
...
func void G12_OpenInventoryEvent_Init()
{
	...
	if (!once)
	{
		HookEngine (7374544, 7, "_hook_oCItemContainer_OpenPassive");
		once = 1;
	};
};
...
{
	MEM_CopyBytes(9258472, trafoPtr, 64);
};
...
func void RotateTrafoY (var int trafoPtr, var int degrees)
{
	...
	if (CALL_Begin(call))
	{
		...
		CALL__thiscall(_@(trafoPtr), 5339008);
		call = CALL_End();
	};
};
...
func void PosDirToTrf(var int posPtr, var int dirPtr, var int trafoPtr)
{
	...
	if (dirPtr)
	{
		...
		if (CALL_Begin(call))
		{
			CALL__thiscall(_@(trafoPtr), 5337904);
			call = CALL_End();
		};
	};
};
...
func int GetGroundLvlPos (var int posPtr)
{
	...
	if (CALL_Begin(call))
	{
		...
		CALL__fastcall(_@(worldPtr), _@(posPtr), 6429568);
		call = CALL_End();
	};
...
func int SearchWaypointByName (var string waypoint)
{
	...
	if (Call_Begin(call))
	{
		CALL__fastcall(_@(waynetPtr), _@(wpNamePtr), 8061744);
		call = CALL_End();
	};
...
func void SetWPToFloor(var int wpPtr)
{
	...
	if (CALL_Begin(call))
	{
		...
		CALL__thiscall(_@(wpPtr), 8061088);
		call = CALL_End();
	};
};
...
func void AlignVobAt (var int vobPtr, var int trfPtr)
{
	...
	if (CALL_Begin(call))
	{
		...
		CALL__thiscall(_@(vobPtr), 6405248);
		call = CALL_End();
	};
...
func void VobSetVisual (var int vobPtr, var string visual)
{
	...
	if (CALL_Begin(call))
	{
		...
		CALL__thiscall(_@(vobPtr), 6301312);
		call = CALL_End();
	};
};
...
func int CreateNewInstance (var int classDefAddr)
{
	...
	CALL__cdecl(MEM_ReadInt(classDefAddr+64));
	return CALL_RetValAsPtr();
};
...
func int InsertObject (var string class, var string objName, var string visual, var int trfPtr, var int parentPtr)
{
	...
	if (CALL_Begin(call))
	{
		...
		CALL__thiscall(_@(worldPtr), 7863856);
		call = CALL_End();
	};
	...
	{
		CALL_IntParam(1);
		CALL__thiscall(vobPtr, MEM_ReadInt(vob._vtbl+12));
	};
...
func void FillMobContainer (var int mobPtr, var string contents)
{
	...
	if (CALL_Begin(call))
	{
		...
		CALL__thiscall(_@(mobPtr), 7496080);
		call = CALL_End();
	};
};
