...
{
	var int DexToSteal;
	DexToSteal = (oth.level * 2) + (oth.aivar[AIV_PlayerHasPickedMyPocket] * 20);
...
func void MOD_Pickpocket (var C_NPC slf, var C_NPC oth)
{
	...
	if (slf.attribute[ATR_DEXTERITY] >= DexToSteal)
	{
		...
		else if (Npc_HasItems(oth, ItMi_Gold) > 0)
		{
			...
			var string GoldCount; GoldCount = ConcatStrings(IntToString(GoldToSteal), PRINT_GoldErhalten);
			Print(GoldCount);
