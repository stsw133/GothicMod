...
func void DIA_CITYGUIDE_Info()
{
	...
	if		(C_NpcIsInQuarter(self) == Q_UNTERSTADT)	{	B_Say (self, other, "$UNTERSTADT");	}	//Jesteœ teraz w dolnej czêœci miasta.
	else if	(C_NpcIsInQuarter(self) == Q_OBERSTADT)		{	B_Say (self, other, "$OBERSTADT");	}	//Jesteœ teraz w górnym mieœcie.
	else if	(C_NpcIsInQuarter(self) == Q_TEMPEL)		{	B_Say (self, other, "$TEMPEL");		}	//Jesteœ teraz przy œwi¹tyni.
	else if	(C_NpcIsInQuarter(self) == Q_MARKT)			{	B_Say (self, other, "$MARKT");		}	//Jesteœ teraz na targowisku.
	else if	(C_NpcIsInQuarter(self) == Q_GALGEN)		{	B_Say (self, other, "$GALGEN");		}	//Jesteœ teraz na placu wisielców.
	else if (C_NpcIsInQuarter(self) == Q_KASERNE)		{	B_Say (self, other, "$KASERNE");	}	//To s¹ koszary.
	else if (C_NpcIsInQuarter(self) == Q_HAFEN)			{	B_Say (self, other, "$HAFEN");		};	//Jesteœ teraz w dzielnicy portowej.
