...
func int C_NpcIsInQuarter (var C_NPC slf)
{
	...
	if (C_IsQuarterNearest(slf, Q_MARKT))		{	return Q_MARKT;			};
	if (C_IsQuarterNearest(slf, Q_TEMPEL))		{	return Q_TEMPEL;		};
	if (C_IsQuarterNearest(slf, Q_UNTERSTADT))	{	return Q_UNTERSTADT;	};
	if (C_IsQuarterNearest(slf, Q_HAFEN))		{	return Q_HAFEN;			};
