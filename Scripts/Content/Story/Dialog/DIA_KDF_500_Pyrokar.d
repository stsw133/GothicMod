...
func void DIA_Pyrokar_OATH_Info ()
{
	...
	CreateInvItems 		(hero,ITAR_MAG_L,1);
	AI_EquipArmor		(hero,ITAR_MAG_L);		
	...
	if (hero.guild == GIL_KDF)
		{
			...
			CreateInvItem	(hero, ITAR_AMG_L);
			AI_EquipArmor	(hero, ITAR_AMG_L);	
...
func void DIA_Pyrokar_SCOBSESSED_Info ()
{
	...
	&& (Npc_HasItems (other,ItPo_HealObsession) == FALSE)  
		{
			if (hero.guild == GIL_KDF)
				{
					...
					CreateInvItems (self, ItPo_HealObsession, 2);									
					B_GiveInvItems (self, other, ItPo_HealObsession, 2);					
