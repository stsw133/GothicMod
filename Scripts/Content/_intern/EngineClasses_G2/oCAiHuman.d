...
class zCAIPlayer
{
	...
	// Config
	// struct zTConfig {
	// Approximately what is also specified in Species.d.
	...
	//
	Just because it's going uphill doesn't mean the NPCs are standing at the same angle
	as the ground is sloped. At least, that's not the case for all NPCs.
	This enum contains the constants that represent the different possibilities.
	...
	/*
	// Structure with information on accessible climbing edges.
	truct zTLedgeInfo{
		zVEC3 point;            // Position to get stuck in
		zVEC3 normal;           // a normal vector to which the figure must align itself
		zVEC3 cont;
	...
	*/
	...
	var int waterLevel;             //int //0: low enough, 1: already wading, 2: swimming
	...
	var int bitfield[3]; //For meaning, see the structure above.
...
class oCAniCtrl_Human
{
	...
	var int bitfield; //as in the struct in the comment
	...
	///Since oCAIHuman (a subclass) doesn't contain much additional information, I've just tacked this on:
