...
class oCGame
{
	...
	/// Views are channels through which the engine can display information.
	/// The standard dialogue subtitle boxes are an example.
	...
	var int gameInfo;               //oCGameInfo*
	var int pl_light;               //zCVobLight*
	var int pl_lightval;            //zREAL
	
	/// Timer
	var int wldTimer;               //oCWorldTimer*
	var int timeStep;               //zREAL                               // Enables single-frame switching
	var int singleStep;             //zBOOL
	
	/// References to individual item classes
	...
	/// Short pause after loading to allow system resources to recover
	...
	/// Object routines ("Wld_SetObjectRoutine"), e.g., streetlights: only on at night
	/*
	typedef struct {
		zSTRING     objName;
		int         stateNum;
	...
	/// Not every barrel in the world has a private visual.
	/// I'm guessing all the visuals used are collected here, and Vobs only keep copies of the references.
...
class CGameManager
{
	...
	var int	playTime;               //int  //is rarely updated (?). At least when saving and loading.
};
