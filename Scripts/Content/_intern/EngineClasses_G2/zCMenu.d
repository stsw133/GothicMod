...
class zCMenu
{
	...
	/// [oCMenu_Status_only]
	/// oCMenu_Status is a subclass and additionally has these properties:
	/// I saved myself the trouble of writing a separate class for these properties, see below:
...
class zCViewText
{
	...
	var string text;    //zSTRING //The decisive characteristic.
	var int font;       //zCFont*
	var int timer;      //zREAL //How do I display the remaining time for PrintScreen that only lasts a certain amount of time?
	var int inPrintWin; //zBOOL //Presumably for ads with "Print" that are pushed upwards.
	
	var int color;      //zCOLOR
	var int timed;      //zBOOL
	var int colored;    //zBOOL
};

// See constants for the address of the global zCView screen.
/* zCViews encapsulate two-dimensional objects on the screen.
 * These include, for example, text or menu items.
 * Menu items behave as follows:
   * A menu item is generated the first time it is used. It then persists until Gothic is closed.
   * A menu item stores its associated text in zCMenuItem.m_listLines.
   * As soon as a menu item needs to display something, it creates an "InnerWindow" and stores a reference to this InnerWindow in zCMenuItem.m_pInnerWindow.
   * The InnerWindow is a zCView and receives a copy of the text to be displayed from the menu item.
   * The InnerWindow stores all text lines (usually one) in zCView.textLines. The "zeroth" element of such a list always remains empty.
   * The text is then drawn in each frame by the InnerWindow. As soon as the menu item decides it no longer needs to display anything, the InnerWindow is destroyed.
*/
...
{
	var int _vtbl;
	var int _zCInputCallBack_vtbl; /// Another _vtbl, because zCView inherits from two classes.
	...
	/// The menu items often use virtual coordinates.
	var int vposx;              //int
	var int vposy;              //int
	var int vsizex;             //int
	var int vsizey;             //int
	
	/// But also "real" pixel positions
	var int pposx;              //int
	var int pposy;              //int
	var int psizex;             //int
	var int psizey;             //int
...
class zCMenuItemText
{
	...
	var int	m_numOptions;       //int //Relevant for options menu items where, for example, you can choose between "off" and "on".
