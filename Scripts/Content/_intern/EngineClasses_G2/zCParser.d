...
class zCParser
{
	...
	//zCPar_Stack               stack;                  // the memory area where the code is located
	...
	var int datastack_stack[/*zMAX_SYM_DATASTACK*/ 2048];     //0x0058 // very small, but it doesn't matter if used correctly
	var int datastack_sptr;                                   //0x085C int
	
	var int _self;                                      //zCPar_Symbol*
	
	/// This part is irrelevant; it happens during parsing itself:
...
const int zCPar_Symbol_bitfield_ele         = ((1 << 12) - 1) <<  0; /// number of elements (in arrays)
const int zCPar_Symbol_bitfield_type        = ((1 <<  4) - 1) << 12; /// see enum below
const int zCPar_Symbol_bitfield_flags       = ((1 <<  6) - 1) << 16; /// see enum below
...
class zCPar_Symbol
{
	var string name;                        //0x0000 zSTRING        // name of the symbol, as it appears in the script
	var int next;                           //0x0014 zCPar_Symbol*  // the symbols appear to be linked. This seems useless to me, since there's already a symbol table
	
	/// Content of the symbol, pointer or primitive type
	...
	var int offset;                         //0x001C Offset for class variables // Address for instances // Return value for functions
	
	var int bitfield;                       //0x0020 see above
...
/* Operators take two values ??from the data stack and perform a calculation.
 * They push the result onto the data stack. */
...
/// Exception:
const int zPAR_OP_IS        = 9;            //"=" 0x09 //Retrieve i1 and i2 from the data stack and set i1 = i2. i1 must be a reference here.
...
/* Unary operators only take one value from the data stack.
   They also push the result onto the data stack. */
...
/* Other commands (Don't forget: operators are also commands!)
 * Some commands are always followed by a parameter.
 * Other commands consist only of the command token */
...
/// Some internal information. Probably only of limited relevance.
...
/// For pushing array values.
const int zPAR_TOK_FLAGARRAY      = zPAR_TOK_VAR | 128; //Do not use!
const int zPAR_TOK_PUSH_ARRAYVAR  = zPAR_TOK_FLAGARRAY + zPAR_TOK_PUSHVAR; //Get the 4-byte symbol index from the stack. Get the 1-byte array index from the stack. Push the address of the corresponding array element in the symbol onto the stack.

/* See also this post:
 * http://forum.worldofplayers.de/forum/showthread.php?p=13086904&#post13086904
 * where I suggest how one could learn to understand the parser. */
